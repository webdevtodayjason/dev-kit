...
description: >-
  Node.js / Bun / TypeScript house style for your projects — runtime
  choice, imports, naming, errors, async patterns, testing.

  Load this skill whenever a worker is touching `.ts`, `.tsx`, `.js`, or
  `.mjs` files in a your project. Use it instead of guessing the
  ...
  expressed across the house conventions.
loaded_by_default: []
...
Default ecosystem skill for Node-flavored work in your projects. These are
the conventions these house defaults were shaped; treat them as the defaults
...
// yes
throw new Error("config missing: ./config/policy.json")
```
...
```
feat(auth): add multi-backend secret resolution
fix(jobs): resolve reconciliation race
test(eval): add coverage for the new modules
...
- Project-specific layout — read the
  project's CLAUDE.md

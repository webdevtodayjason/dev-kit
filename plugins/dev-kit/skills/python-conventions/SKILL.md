...
description: >-
  Python house style for general engineering work — tooling (uv, ruff, pytest),
  imports, types, errors, async patterns, packaging.

  Load this skill whenever a worker is touching `.py` files in a
  your project. Most your projects are Bun-first, but supporting
...
Default ecosystem skill for Python-flavored work. Most projects here are Bun, but
...
- Same `[scope]` prefix convention as the Node side, for dashboard log
  filtering when these logs route through to a central sink.
...
- Jupyter notebooks — see project conventions; most projects don't ship notebooks

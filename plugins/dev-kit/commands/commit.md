...
- `--no-verify`: Skip pre-commit checks (lint, build, etc.)
- `--style=simple|full`: 
...
1. **Pre-commit checks** (unless `--no-verify`):
   - Lint (e.g. `npm run lint`, `pnpm lint`, or `bun run lint`) - ensure code quality
   - Build (e.g. `npm run build`) - verify build succeeds
   - Any other repo-defined pre-commit scripts (tests, docs generation, etc.)
